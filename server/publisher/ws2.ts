...
export class Ws2Publisher {
    ...
    onNewBlock() {
        const output = { blockheight: DataStorage.lastBlock }
    ...
    onNewFee() {
        if (this.lastRoundFee != Math.round(DataStorage.lastMedianFee)) {
            const output = { blockfee: Math.round(DataStorage.lastMedianFee) }
        ...
        const output2 = { blockfee2: Math.round(DataStorage.lastMedianFee * 100) / 100 }
