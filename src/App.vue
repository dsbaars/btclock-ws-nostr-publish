...
<template class="pure-g">
    <nav class="navbar bg-body-tertiary">
        <div class="container-fluid">
            <span class="navbar-brand mb-0 h1">BTClock WebSocket Data Server</span>
        </div>
    </nav>
    <form class="container-fluid" id="dataForm">
        ...
        <fieldset>
            <div class="data-form">
                <div class="row g-1 gx-4">
                    <div class="col-md">
                        <div class="input-group input-group-sm ">
                            <div class="input-group-text w-50">Price</div>
                            <input type="number" class="form-control" id="data-price" v-model="currentPrice" min="0"
                                placeholder="Price" />
                        </div>
                    </div>
                    <div class="col-md">
                        <div class="input-group input-group-sm ">
                            <div class="input-group-text  w-50" for="data-blockHeight">Block Height</div>
                            <input type="number" class="form-control" id="data-blockHeight" v-model="blockHeight"
                                min="0" placeholder="Block Height" />
                        </div>
                    </div>
                    <div class="col-md">
                        <div class="input-group input-group-sm ">
                            <div class="input-group-text  w-50" for="data-feeRate">Fee Rate</div>
                            <input type="number" class="form-control" id="data-feeRate" v-model="feeRate" min="0"
                ...
                <div class="row g-1 gx-4">
                    <div class="col-md">
                        <div class="form-check">
                            <input type="checkbox"  class="form-check-input"  v-model="ignoreDataSource" id="ignore-external-data" /> &nbsp;
                            <label for="ignore-external-data" class="form-check-label">Ignore data source</label>
                        </div>
                    </div>
                    <div class="col-md">
                        <div class="form-check">
                            <input type="checkbox"  class="form-check-input"  v-model="showOtherCurrencies" id="show-other-currencies" /> &nbsp;
                            <label for="show-other-currencies" class="form-check-label">Show other currencies</label>
                        </div>
                    </div>
                    <div class="col-md">
                        <div class="form-check">
                            <input type="checkbox"  class="form-check-input"  v-model="showSatsSymbol" id="show-sats-symbol" /> &nbsp;
    ...
    <div class="container-fluid">
        <div class="row">
            <div id="terminalContainer" class="termContainer">
                <WebsocketTerminal componentId="websocketConnection" :socket="socket1" :version="'v1 (JSON) '"
                    ref="wsTerminal"></WebsocketTerminal>
            </div>
            <div id="terminal2Container" class="termContainer">
                <WebsocketTerminal componentId="websocketConection2" :socket="socket2" :version="'v2 (MsgPack) '"
                    ref="wsTerminal2"></WebsocketTerminal>
            </div>
    ...
    <div class="preview-container" v-if="showOtherCurrencies">
        <BTClock :data="currentPriceOther.EUR" method="parsePriceData" :params="[CURRENCY_EUR, false, false, false]">
        </BTClock>
        <BTClock :data="currentPriceOther.EUR" method="parseSatsPerCurrency" :params="[CURRENCY_EUR, showSatsSymbol]">
        </BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPriceOther.EUR, CURRENCY_EUR, false]">
        </BTClock>

        <BTClock :data="currentPriceOther.GBP" method="parsePriceData" :params="[CURRENCY_GBP, false, false, false]">
        </BTClock>
        <BTClock :data="currentPriceOther.GBP" method="parseSatsPerCurrency" :params="[CURRENCY_GBP, showSatsSymbol]">
        </BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPriceOther.GBP, CURRENCY_GBP, false]">
        </BTClock>

        <BTClock :data="currentPriceOther.JPY" method="parsePriceData" :params="[CURRENCY_JPY, false, false, false]">
        </BTClock>
        <BTClock :data="currentPriceOther.JPY" method="parseSatsPerCurrency" :params="[CURRENCY_JPY, showSatsSymbol]">
        </BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPriceOther.JPY, CURRENCY_JPY, false]">
        </BTClock>

        <BTClock :data="currentPriceOther.AUD" method="parsePriceData" :params="[CURRENCY_AUD, false, false, false]">
        </BTClock>
        <BTClock :data="currentPriceOther.AUD" method="parseSatsPerCurrency" :params="[CURRENCY_AUD, showSatsSymbol]">
        </BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPriceOther.AUD, CURRENCY_AUD, false]">
        </BTClock>

        <BTClock :data="currentPriceOther.CAD" method="parsePriceData" :params="[CURRENCY_CAD, false, false, false]">
        </BTClock>
        <BTClock :data="currentPriceOther.CAD" method="parseSatsPerCurrency" :params="[CURRENCY_CAD, showSatsSymbol]">
        </BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPriceOther.CAD, CURRENCY_CAD, false]">
        </BTClock>

    </div>
    <div class="preview-container">
        <BTClock :data="blockHeight" method="parseBlockHeight" title="Block Height"></BTClock>
        <BTClock :data="blockHeight" method="parseBitcoinSupply" :params="[true, false]"  title="BTC Supply (absolute)"></BTClock>
        <BTClock :data="blockHeight" method="parseBitcoinSupply" :params="[true, true]"  title="BTC Supply (percentage) "></BTClock>
        ...
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPrice, '$', false]" title="Market Cap (small chars)"></BTClock>
        <BTClock :data="blockHeight" method="parseMarketCap" :params="[currentPrice, '$', true]" title="Market Cap (big chars)"></BTClock>
        <BTClock :data="currentPrice" method="parsePriceData" :params="['$', true, false, true]" title="Ticker (Suffix notation, compact)"></BTClock>
        <BTClock :data="currentPrice" method="parsePriceData" :params="['$', true, false, false]" title="Ticker (Suffix notation)"></BTClock>
        <BTClock :data="currentPrice" method="parsePriceData" :params="['$', false, false, false]" title="Ticker (Default)"></BTClock>
        <BTClock :data="currentPrice" method="parsePriceData" :params="['$', true, true, true]" title="Ticker (Mow Suffix notation, compact)"></BTClock>
        <BTClock :data="currentPrice" method="parsePriceData" :params="['$', true, true, false]" title="Ticker (Mow Suffix notation)"></BTClock>
    </div>
